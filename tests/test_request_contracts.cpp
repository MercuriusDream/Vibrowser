...
int main() {
    ...
    // Test 11: status-line parser captures HTTP/1.x protocol versions and rejects unsupported transports
    {
        ...
        if (browser::net::parse_http_status_line(
                " HTTP/1.1 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with leading whitespace to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for leading-whitespace variant\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with leading whitespace is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1 200 OK ", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with trailing whitespace to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for trailing-whitespace variant\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with trailing whitespace is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                "HTTP/1.1  200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with extra status-code separator whitespace to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for extra status-code separator whitespace\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with extra status-code separator whitespace is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                "HTTP/1.1\f200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with form-feed status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for form-feed status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with form-feed status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1 200\fOK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with form-feed reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for form-feed reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with form-feed reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1\v200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with vertical-tab status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for vertical-tab status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with vertical-tab status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1 200\vOK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with vertical-tab reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for vertical-tab reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with vertical-tab reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1\r200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with carriage-return status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for carriage-return status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with carriage-return status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1 200\rOK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with carriage-return reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for carriage-return reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with carriage-return reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1\n200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with line-feed status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for line-feed status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with line-feed status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.1 200\nOK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with line-feed reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for line-feed reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with line-feed reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\000200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with NUL status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for NUL status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with NUL status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\000OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with NUL reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for NUL reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with NUL reason separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\002200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with STX status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for STX status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with STX status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\002OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with STX reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for STX reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with STX reason separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\302200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with non-ASCII status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for non-ASCII status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with non-ASCII status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\302OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with non-ASCII reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for non-ASCII reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with non-ASCII reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\016200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SO status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SO status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SO status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\016OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SO reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SO reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SO reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\017200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SI status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SI status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SI status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\017OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SI reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SI reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SI reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\020200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DLE status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DLE status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DLE status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\020OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DLE reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DLE reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DLE reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\021200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC1 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC1 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC1 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\021OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC1 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC1 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC1 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\022200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC2 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC2 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC2 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\022OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC2 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC2 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC2 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\023200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC3 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC3 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC3 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\023OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC3 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC3 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC3 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\024200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC4 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC4 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC4 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\024OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with DC4 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for DC4 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with DC4 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\025200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with NAK status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for NAK status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with NAK status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\025OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with NAK reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for NAK reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with NAK reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\026200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SYN status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SYN status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SYN status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\026OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SYN reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SYN reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SYN reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\027200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with ETB status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for ETB status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with ETB status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\027OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with ETB reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for ETB reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with ETB reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\030200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with CAN status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for CAN status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with CAN status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\030OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with CAN reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for CAN reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with CAN reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\031200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with EM status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for EM status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with EM status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\031OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with EM reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for EM reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with EM reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\032200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SUB status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SUB status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SUB status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\032OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with SUB reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for SUB reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with SUB reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\033200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with ESC status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for ESC status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with ESC status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\033OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with ESC reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for ESC reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with ESC reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\034200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with FS status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for FS status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with FS status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\034OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with FS reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for FS reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with FS reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\035200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with GS status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for GS status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with GS status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\035OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with GS reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for GS reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with GS reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\036200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with RS status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for RS status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with RS status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\036OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with RS reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for RS reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with RS reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\037200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with US status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for US status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with US status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\037OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with US reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for US reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with US reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\x80" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0x80 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0x80 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0x80 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\x80" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0x80 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0x80 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0x80 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\xa0" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xA0 status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xA0 status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xA0 status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\xa0" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xA0 reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xA0 reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xA0 reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\x9f" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0x9F status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0x9F status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0x9F status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\x9f" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0x9F reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0x9F reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0x9F reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\xff" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFF status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFF status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFF status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\xff" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFF reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFF reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFF reason separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\xfe" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFE status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFE status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFE status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\xfe" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFE reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFE reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFE reason separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1\xfc" "200 OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFC status-code separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFC status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFC status-code separator is rejected\n";
        }

        if (browser::net::parse_http_status_line(
                std::string("HTTP/1.1 200\xfc" "OK", 15), http_version, status_code, reason, err)) {
            std::cerr << "FAIL: expected status line with 0xFC reason separator to be rejected\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed status-line error for 0xFC reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line with 0xFC reason separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
            ...
            std::cerr << "FAIL: expected malformed status-line error for missing reason separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line missing reason separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
            ...
            std::cerr << "FAIL: expected malformed status-line error for missing status-code separator\n";
            ++failures;
        } else {
            std::cerr << "PASS: status line missing status-code separator is rejected\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                "HTTP/1.10 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/1.10 status line should be rejected as malformed\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed error for HTTP/1.10; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/1.10 status line is rejected as malformed\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.11 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/1.11 status line should be rejected as malformed\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed error for HTTP/1.11; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/1.11 status line is rejected as malformed\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1.00 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/1.00 status line should be rejected as malformed\n";
            ++failures;
        } else if (err.find("Malformed HTTP status line") == std::string::npos) {
            std::cerr << "FAIL: expected malformed error for HTTP/1.00; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/1.00 status line is rejected as malformed\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/1 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/1 status line should be rejected as unsupported version\n";
            ++failures;
        } else if (err.find("Unsupported HTTP status line version 'HTTP/1'") == std::string::npos) {
            std::cerr << "FAIL: expected unsupported-version error for HTTP/1; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/1 status line is rejected as unsupported version\n";
        }
        ...
        if (browser::net::parse_http_status_line(
                "HTTP/0.9 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/0.9 status line should be rejected as unsupported version\n";
            ++failures;
        } else if (err.find("Unsupported HTTP status line version 'HTTP/0.9'") == std::string::npos) {
            std::cerr << "FAIL: expected unsupported-version error for HTTP/0.9; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/0.9 status line is rejected as unsupported version\n";
        }

        if (browser::net::parse_http_status_line(
                "HTTP/2.1 200 OK", http_version, status_code, reason, err)) {
            std::cerr << "FAIL: HTTP/2.1 status line should be rejected as unsupported version\n";
            ++failures;
        } else if (err.find("Unsupported HTTP status line version 'HTTP/2.1'") == std::string::npos) {
            std::cerr << "FAIL: expected unsupported-version error for HTTP/2.1; got: " << err << "\n";
            ++failures;
        } else {
            std::cerr << "PASS: HTTP/2.1 status line is rejected as unsupported version\n";
        }
