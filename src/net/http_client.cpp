...
bool parse_status_line(const std::string& status_line, Response& response, std::string& err) {
  ...
  if ((starts_with(version, "HTTP/1.0") && version.size() > 8) ||
      (starts_with(version, "HTTP/1.1") && version.size() > 8)) {
    err = "Malformed HTTP status line";
    return false;
  }
  ...
  for (std::size_t i = 0; i <= upgrade_header.size(); ++i) {
    ...
    if (!token.empty()) {
      ...
      }
      {
        ...
        unescaped_token.reserve(token.size());
        bool unescape_next = false;
        for (char token_ch : token) {
          if (unescape_next) {
            unescaped_token.push_back(token_ch);
            unescape_next = false;
            continue;
          }
          if (token_ch == '\\') {
        ...
        if (unescape_next) {
          unescaped_token.push_back('\\');
        }
        token = trim_ascii(unescaped_token);
      }
      if (token.size() >= 2 && token.front() == '"' && token.back() == '"') {
        token = trim_ascii(token.substr(1, token.size() - 2));
      }
