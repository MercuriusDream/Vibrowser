    ...
    for (const auto& sheet : stylesheets_) {
        for (const auto& rule : sheet.rules) {
            for (const auto& complex_sel : rule.selectors.selectors) {
                ...
                // Check if the last compound selector contains a pseudo-element
                // matching pseudo_name
                ...
                // Build a modified complex selector without the pseudo-element
                // to match against the real element
                ...
                // If the modified selector is empty (e.g., "::before" with no
                // element part), we need at least a universal match
                bool matches = false;
                if (mod_last.empty() && modified.parts.size() == 1) {
                    // Bare pseudo-element like "::before" - matches any element
                    matches = true;
                } else {
