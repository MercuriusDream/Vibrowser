...
std::unique_ptr<clever::layout::LayoutNode> build_layout_tree_styled(
    ...
    layout_node->geometry.margin.top = style.margin.top.is_auto() ? 0 : style.margin.top.to_px(0);
    layout_node->geometry.margin.right = style.margin.right.is_auto() ? -1 : style.margin.right.to_px(0);
    layout_node->geometry.margin.bottom = style.margin.bottom.is_auto() ? 0 : style.margin.bottom.to_px(0);
    layout_node->geometry.margin.left = style.margin.left.is_auto() ? -1 : style.margin.left.to_px(0);

    // Padding
    layout_node->geometry.padding.top = style.padding.top.to_px(0);
    layout_node->geometry.padding.right = style.padding.right.to_px(0);
    layout_node->geometry.padding.bottom = style.padding.bottom.to_px(0);
    layout_node->geometry.padding.left = style.padding.left.to_px(0);
...
RenderResult render_html(const std::string& html, const std::string& base_url,
    ...
    try {
        ...
        // Step 5: Paint -> DisplayList
        Painter painter;
        auto display_list = painter.paint(*layout_root, static_cast<float>(viewport_height));

        // Step 6: Render to pixel buffer
        // Use actual content height so scrollable content is fully rendered
