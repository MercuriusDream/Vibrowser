...
void Painter::paint_node(const clever::layout::LayoutNode& node, DisplayList& list,
    ...
    if (!node.visibility_hidden) {
        ...
        if (node.image_pixels && !node.image_pixels->empty()) {
            ...
            if (node.image_orientation == 2 && img->width > 0 && img->height > 0) {
                int w = img->width;
                int h = img->height;
                auto& px = img->pixels;
    ...
    if (clipping) {
        list.push_clip({child_offset_x, child_offset_y, geom.width, geom.height});
    }

    // Build paint order: sort children by z-index (lower z-index paints first)
    // Children with the same z-index maintain DOM order (stable sort)
    std::vector<const clever::layout::LayoutNode*> paint_order;
    paint_order.reserve(node.children.size());
    for (auto& child : node.children) {
        paint_order.push_back(child.get());
    }
    if (paint_order.size() > 1) {
        // Only sort if any child has non-zero z-index
        bool has_z = false;
        for (auto* c : paint_order) {
            if (c->z_index != 0) { has_z = true; break; }
        }
        if (has_z) {
    ...
    // Recursively paint children in z-index order
...
void Painter::paint_borders(const clever::layout::LayoutNode& node, DisplayList& list,
    ...
    if (same_color && same_style) {
        ...
        if (style == 0 && node.border_style != 0) style = node.border_style;
