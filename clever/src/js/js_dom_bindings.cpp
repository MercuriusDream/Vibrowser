...
static clever::html::SimpleNode* query_selector_impl(
...
static void query_selector_all_impl(
        clever::html::SimpleNode* root, const std::string& selector,
        std::vector<clever::html::SimpleNode*>& results) {
...
(function() {
    ...
    // ------------------------------------------------------------------
    // window.devicePixelRatio and other common window properties
    // ------------------------------------------------------------------
    JS_SetPropertyStr(ctx, global, "devicePixelRatio", JS_NewFloat64(ctx, 2.0));
    JS_SetPropertyStr(ctx, global, "innerWidth", JS_NewInt32(ctx, 1024));
    JS_SetPropertyStr(ctx, global, "innerHeight", JS_NewInt32(ctx, 768));
    JS_SetPropertyStr(ctx, global, "outerWidth", JS_NewInt32(ctx, 1024));
    JS_SetPropertyStr(ctx, global, "outerHeight", JS_NewInt32(ctx, 768));
...
(function() {
    globalThis.matchMedia = function(query) {
        ...
        if (query.indexOf('(min-width:') !== -1) {
            ...
            if (m) matches = parseInt(m[1]) <= 1024;
        }
...
(function() {
    ...
    globalThis.find = function() { return false; };
    globalThis.open = function() { return null; };
    globalThis.close = function() {};
    globalThis.postMessage = function() {};
    globalThis.requestIdleCallback = function(fn) { fn({timeRemaining: function() { return 50; }, didTimeout: false}); return 1; };
...
void fire_resize_observers(JSContext* ctx, int viewport_w, int viewport_h) {
    ...
    for (auto& ro : state->resize_observers) {
        ...
        for (auto* elem : ro.observed_elements) {
            ...
            // Uses content box * devicePixelRatio (2.0 on Retina)
            JSValue dpbs_arr = JS_NewArray(ctx);
            JSValue dpbs = JS_NewObject(ctx);
            JS_SetPropertyStr(ctx, dpbs, "inlineSize", JS_NewFloat64(ctx, content_w * 2.0f));
            JS_SetPropertyStr(ctx, dpbs, "blockSize", JS_NewFloat64(ctx, content_h * 2.0f));
