...
struct SimpleNode {
    ...
    std::unique_ptr<SimpleNode> take_child(SimpleNode* child);
...
private:
    ...
    std::vector<SimpleNode*> active_formatting_elements_;
    void push_active_formatting_element(SimpleNode* node);
    void remove_active_formatting_element(SimpleNode* node);
    SimpleNode* find_active_formatting_element(const std::string& tag) const;
    void reconstruct_active_formatting_elements();
    void handle_formatting_element(const Token& token);
    void run_adoption_agency(const std::string& tag);
