...
// 1. Round-trip u8 boundary values with interleaved bools
TEST(SerializerTest, U8InterleavedWithBoolV106) {
    Serializer s;
    s.write_u8(0);
    s.write_bool(false);
    s.write_u8(128);
...
// 2. Multiple empty strings followed by a non-empty string
TEST(SerializerTest, MultipleEmptyStringsThenNonEmptyV106) {
    Serializer s;
    s.write_string("");
    s.write_string("");
    s.write_string("");
    s.write_string("finally");

    Deserializer d(s.data());
    EXPECT_EQ(d.read_string(), "");
    EXPECT_EQ(d.read_string(), "");
    EXPECT_EQ(d.read_string(), "");
    EXPECT_EQ(d.read_string(), "finally");
    EXPECT_FALSE(d.has_remaining());
}

// 3. u64 max and min interleaved with i64 extremes
TEST(SerializerTest, U64MaxMinWithI64ExtremesV106) {
    Serializer s;
    s.write_u64(std::numeric_limits<uint64_t>::max());
    s.write_i64(std::numeric_limits<int64_t>::min());
    s.write_u64(0ULL);
...
// 4. write_bytes with zero-length payload
TEST(SerializerTest, WriteBytesZeroLengthV106) {
    Serializer s;
    s.write_u32(0xDEADBEEFu);
    uint8_t dummy = 0;
...
// 5. f64 special values: positive infinity, negative infinity, zero, negative zero
TEST(SerializerTest, F64SpecialValuesV106) {
    Serializer s;
    s.write_f64(std::numeric_limits<double>::infinity());
    s.write_f64(-std::numeric_limits<double>::infinity());
    s.write_f64(0.0);
...
// 6. Large string round-trip (10000 chars)
TEST(SerializerTest, LargeStringRoundTripV106) {
    std::string big(10000, 'X');
    big[0] = 'A';
    big[9999] = 'Z';
    ...
    EXPECT_EQ(result[9999], 'Z');
    EXPECT_EQ(result[5000], 'X');
    EXPECT_FALSE(d.has_remaining());
}

// 7. Alternating u16 and u32 values
TEST(SerializerTest, AlternatingU16U32V106) {
    Serializer s;
    for (uint32_t i = 0; i < 20; ++i) {
        s.write_u16(static_cast<uint16_t>(i * 1000));
    ...
    for (uint32_t i = 0; i < 20; ++i) {
        EXPECT_EQ(d.read_u16(), static_cast<uint16_t>(i * 1000));
        EXPECT_EQ(d.read_u32(), i * 100000u);
    }
    EXPECT_FALSE(d.has_remaining());
}

// 8. Mixed types: every type in one serializer
TEST(SerializerTest, EveryTypeInOneMessageV106) {
    Serializer s;
    s.write_u8(42);
    s.write_u16(12345);
    s.write_u32(3000000000u);
    s.write_u64(9999999999999ULL);
    s.write_i32(-2147483647);
    s.write_i64(-8000000000000LL);
    s.write_f64(2.718281828459045);
    s.write_bool(true);
    s.write_string("hello world");
    ...
    EXPECT_EQ(d.read_u8(), 42);
    EXPECT_EQ(d.read_u16(), 12345);
    EXPECT_EQ(d.read_u32(), 3000000000u);
    EXPECT_EQ(d.read_u64(), 9999999999999ULL);
    EXPECT_EQ(d.read_i32(), -2147483647);
    EXPECT_EQ(d.read_i64(), -8000000000000LL);
    EXPECT_DOUBLE_EQ(d.read_f64(), 2.718281828459045);
    EXPECT_EQ(d.read_bool(), true);
    EXPECT_EQ(d.read_string(), "hello world");
    auto read_blob = d.read_bytes();
    ASSERT_EQ(read_blob.size(), 4u);
    EXPECT_EQ(read_blob[0], 0x01);
    EXPECT_EQ(read_blob[1], 0x02);
    EXPECT_EQ(read_blob[2], 0x03);
    EXPECT_EQ(read_blob[3], 0xFF);
    EXPECT_FALSE(d.has_remaining());
}
...
TEST(SerializerTest, U8BoundaryValuesV107) {
    Serializer s;
    s.write_u8(0x00);
    s.write_u8(0x01);
    s.write_u8(0x7F);
...
TEST(SerializerTest, U16EndianConsistencyV107) {
    Serializer s;
    s.write_u16(0x0100);
    s.write_u16(0xFF00);
    s.write_u16(0x00FF);
...
TEST(SerializerTest, I32NegativeBoundariesV107) {
    Serializer s;
    s.write_i32(std::numeric_limits<int32_t>::min());
    s.write_i32(std::numeric_limits<int32_t>::max());
    s.write_i32(-1);
...
TEST(SerializerTest, I64LargeNegativeAndPositiveV107) {
    Serializer s;
    s.write_i64(std::numeric_limits<int64_t>::min());
    s.write_i64(std::numeric_limits<int64_t>::max());
    s.write_i64(-1LL);
    s.write_i64(0LL);
    s.write_i64(4294967296LL);
    s.write_i64(-4294967296LL);
    ...
    EXPECT_EQ(d.read_i64(), -1LL);
    EXPECT_EQ(d.read_i64(), 0LL);
    EXPECT_EQ(d.read_i64(), 4294967296LL);
    EXPECT_EQ(d.read_i64(), -4294967296LL);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, F64SpecialIeeeValuesV107) {
    Serializer s;
    s.write_f64(0.0);
    s.write_f64(-0.0);
    s.write_f64(std::numeric_limits<double>::infinity());
...
TEST(SerializerTest, StringWithEmbeddedNullsV107) {
    Serializer s;
    std::string with_nulls("abc\0def", 7);
    s.write_string(with_nulls);
    s.write_string("");
    s.write_string("normal");
    ...
    EXPECT_EQ(result, with_nulls);
    EXPECT_EQ(d.read_string(), "");
    EXPECT_EQ(d.read_string(), "normal");
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BytesEmptyAndLargeV107) {
    Serializer s;
    // Empty bytes
    s.write_bytes(nullptr, 0);
    ...
    Deserializer d(s.data());
    auto empty_blob = d.read_bytes();
    EXPECT_EQ(empty_blob.size(), 0u);
    auto large_blob = d.read_bytes();
    ASSERT_EQ(large_blob.size(), 256u);
    for (size_t i = 0; i < 256; ++i) {
        EXPECT_EQ(large_blob[i], static_cast<uint8_t>(i));
    }
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BoolAndU64InterleavedV107) {
    Serializer s;
    s.write_bool(true);
    s.write_u64(0xDEADBEEFCAFEBABEULL);
    s.write_bool(false);
...
TEST(SerializerTest, U8BoundaryValuesV108) {
    Serializer s;
    s.write_u8(0);
    s.write_u8(1);
    s.write_u8(128);
...
TEST(SerializerTest, U16EndianConsistencyV108) {
    Serializer s;
    s.write_u16(0x0000);
    s.write_u16(0x00FF);
    s.write_u16(0xFF00);
...
TEST(SerializerTest, I32NegativeRoundTripV108) {
    Serializer s;
    s.write_i32(0);
    s.write_i32(-1);
    s.write_i32(std::numeric_limits<int32_t>::min());
    s.write_i32(std::numeric_limits<int32_t>::max());
    s.write_i32(-123456789);
    ...
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::min());
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::max());
    EXPECT_EQ(d.read_i32(), -123456789);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, I64ExtremeValuesV108) {
    Serializer s;
    s.write_i64(std::numeric_limits<int64_t>::min());
    s.write_i64(std::numeric_limits<int64_t>::max());
    s.write_i64(0LL);
...
TEST(SerializerTest, F64SpecialFloatsV108) {
    Serializer s;
    s.write_f64(0.0);
    s.write_f64(-0.0);
    s.write_f64(std::numeric_limits<double>::infinity());
    ...
    Deserializer d(s.data());
    EXPECT_EQ(d.read_f64(), 0.0);
    EXPECT_TRUE(std::signbit(d.read_f64()));  // -0.0
    EXPECT_EQ(d.read_f64(), std::numeric_limits<double>::infinity());
    EXPECT_EQ(d.read_f64(), -std::numeric_limits<double>::infinity());
    EXPECT_TRUE(std::isnan(d.read_f64()));
    EXPECT_DOUBLE_EQ(d.read_f64(), 3.141592653589793);
...
TEST(SerializerTest, StringWithEmbeddedNullsV108) {
    Serializer s;
    std::string with_null("hello\0world", 11);
    s.write_string(with_null);
    s.write_string("");
    ...
    Deserializer d(s.data());
    std::string out1 = d.read_string();
    EXPECT_EQ(out1.size(), 11u);
    EXPECT_EQ(out1, with_null);
    EXPECT_EQ(d.read_string(), "");
    EXPECT_EQ(d.read_string(), "simple");
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BytesBlobRoundTripV108) {
    Serializer s;
    std::vector<uint8_t> blob(512);
    for (size_t i = 0; i < blob.size(); ++i) {
        blob[i] = static_cast<uint8_t>(i % 256);
    }
    s.write_bytes(blob.data(), blob.size());
    // Also write an empty blob
    s.write_bytes(nullptr, 0);

    Deserializer d(s.data());
    auto out_blob = d.read_bytes();
    ASSERT_EQ(out_blob.size(), 512u);
    for (size_t i = 0; i < 512; ++i) {
        EXPECT_EQ(out_blob[i], static_cast<uint8_t>(i % 256));
    }
    auto empty_blob = d.read_bytes();
    EXPECT_EQ(empty_blob.size(), 0u);
    EXPECT_FALSE(d.has_remaining());
...
TEST(SerializerTest, MixedTypeSequentialWritesV108) {
    Serializer s;
    s.write_bool(true);
    s.write_u8(42);
    s.write_u16(1000);
    s.write_u32(70000u);
    s.write_u64(0xAABBCCDDEEFF0011ULL);
    s.write_i32(-999);
    s.write_i64(-8888888888LL);
    s.write_f64(2.718281828);
    s.write_string("interleaved");
    std::vector<uint8_t> tiny = {0xDE, 0xAD};
    s.write_bytes(tiny.data(), tiny.size());
    s.write_bool(false);
    ...
    EXPECT_EQ(d.read_u16(), 1000);
    EXPECT_EQ(d.read_u32(), 70000u);
    EXPECT_EQ(d.read_u64(), 0xAABBCCDDEEFF0011ULL);
    EXPECT_EQ(d.read_i32(), -999);
    EXPECT_EQ(d.read_i64(), -8888888888LL);
    EXPECT_DOUBLE_EQ(d.read_f64(), 2.718281828);
    EXPECT_EQ(d.read_string(), "interleaved");
    auto blob = d.read_bytes();
...
TEST(SerializerTest, U32PowersOfTwoV109) {
    Serializer s;
    for (int i = 0; i < 32; ++i) {
        s.write_u32(1u << i);
    }
...
TEST(SerializerTest, I64AlternatingSignsV109) {
    Serializer s;
    s.write_i64(1LL);
    s.write_i64(-2LL);
    s.write_i64(3LL);
    ...
    Deserializer d(s.data());
    EXPECT_EQ(d.read_i64(), 1LL);
    EXPECT_EQ(d.read_i64(), -2LL);
    EXPECT_EQ(d.read_i64(), 3LL);
    EXPECT_EQ(d.read_i64(), -4LL);
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::max());
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::min());
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, F64SpecialSequenceV109) {
    Serializer s;
    s.write_f64(0.0);
    s.write_f64(-0.0);
    s.write_f64(std::numeric_limits<double>::infinity());
...
TEST(SerializerTest, StringWithEmbeddedNullsV109) {
    Serializer s;
    std::string with_nulls("hello\0world", 11);
    s.write_string(with_nulls);
    s.write_string("");
    ...
    EXPECT_EQ(r1, with_nulls);
    EXPECT_EQ(d.read_string(), "");
    std::string r3 = d.read_string();
    EXPECT_EQ(r3.size(), 1u);
    EXPECT_EQ(r3[0], '\0');
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BytesBinaryPatternV109) {
    Serializer s;
    // Write 256-byte pattern: 0x00..0xFF
    ...
    s.write_bytes(nullptr, 0);
    Deserializer d(s.data());
    auto blob1 = d.read_bytes();
    ASSERT_EQ(blob1.size(), 256u);
    for (int i = 0; i < 256; ++i) {
        EXPECT_EQ(blob1[i], static_cast<uint8_t>(i));
    }
    auto blob2 = d.read_bytes();
    EXPECT_EQ(blob2.size(), 0u);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BoolRunLengthV109) {
    Serializer s;
    // Write 20 bools in a pattern: true,true,false,true,true,false...
    for (int i = 0; i < 20; ++i) {
        s.write_bool(i % 3 != 2);
    }
    Deserializer d(s.data());
    for (int i = 0; i < 20; ++i) {
        EXPECT_EQ(d.read_bool(), (i % 3 != 2));
    }
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, U16U64InterleavedV109) {
    Serializer s;
    s.write_u16(0);
    s.write_u64(0xFFFFFFFFFFFFFFFFULL);
    s.write_u16(0xFFFF);
...
TEST(SerializerTest, ManyStringsAndBytesV109) {
    Serializer s;
    // 10 strings of increasing length
    for (int i = 0; i < 10; ++i) {
        s.write_string(std::string(i * 50, 'A' + i));
    }
    // 5 byte buffers with repeated values
    for (int i = 0; i < 5; ++i) {
        std::vector<uint8_t> buf(i * 100, static_cast<uint8_t>(i));
        s.write_bytes(buf.data(), buf.size());
    }
    Deserializer d(s.data());
    for (int i = 0; i < 10; ++i) {
        std::string expected(i * 50, 'A' + i);
        EXPECT_EQ(d.read_string(), expected);
    }
    for (int i = 0; i < 5; ++i) {
        auto blob = d.read_bytes();
        ASSERT_EQ(blob.size(), static_cast<size_t>(i * 100));
        for (size_t j = 0; j < blob.size(); ++j) {
            EXPECT_EQ(blob[j], static_cast<uint8_t>(i));
        }
    }
    EXPECT_FALSE(d.has_remaining());
}
...
TEST(SerializerTest, U8BoundaryPatternsV110) {
    Serializer s;
    // Write every power-of-two value that fits in u8
    for (int shift = 0; shift < 8; ++shift) {
        s.write_u8(static_cast<uint8_t>(1u << shift));
    }
    // Also write the complement of each
    for (int shift = 0; shift < 8; ++shift) {
        s.write_u8(static_cast<uint8_t>(~(1u << shift) & 0xFF));
    }
    Deserializer d(s.data());
    for (int shift = 0; shift < 8; ++shift) {
        EXPECT_EQ(d.read_u8(), static_cast<uint8_t>(1u << shift));
    }
    for (int shift = 0; shift < 8; ++shift) {
        EXPECT_EQ(d.read_u8(), static_cast<uint8_t>(~(1u << shift) & 0xFF));
    }
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, I32SpecialNegativeValuesV110) {
    Serializer s;
    s.write_i32(std::numeric_limits<int32_t>::min());      // -2147483648
    s.write_i32(std::numeric_limits<int32_t>::min() + 1);  // -2147483647
    s.write_i32(-1);
    s.write_i32(-256);
    s.write_i32(-65536);
    s.write_i32(-16777216);
    Deserializer d(s.data());
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::min());
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::min() + 1);
    EXPECT_EQ(d.read_i32(), -1);
    EXPECT_EQ(d.read_i32(), -256);
    EXPECT_EQ(d.read_i32(), -65536);
    EXPECT_EQ(d.read_i32(), -16777216);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, F64SpecialIEEEValuesV110) {
    Serializer s;
    s.write_f64(std::numeric_limits<double>::infinity());
    s.write_f64(-std::numeric_limits<double>::infinity());
    s.write_f64(std::numeric_limits<double>::quiet_NaN());
...
TEST(SerializerTest, BoolAlternatingPatternV110) {
    Serializer s;
    // Write 20 alternating booleans
    for (int i = 0; i < 20; ++i) {
        s.write_bool(i % 2 == 0);
    }
    Deserializer d(s.data());
...
TEST(SerializerTest, StringWithEmbeddedNullsV110) {
    Serializer s;
    std::string with_nulls("hello\0world\0end", 15);
    s.write_string(with_nulls);
    s.write_string(std::string(1, '\0'));
    s.write_string(std::string(5, '\0'));
    Deserializer d(s.data());
    std::string r1 = d.read_string();
    EXPECT_EQ(r1.size(), 15u);
    EXPECT_EQ(r1, with_nulls);
    std::string r2 = d.read_string();
    EXPECT_EQ(r2.size(), 1u);
    EXPECT_EQ(r2[0], '\0');
    std::string r3 = d.read_string();
    EXPECT_EQ(r3.size(), 5u);
    for (char c : r3) {
        EXPECT_EQ(c, '\0');
    }
    EXPECT_FALSE(d.has_remaining());
...
TEST(SerializerTest, BytesIotaPatternV110) {
    Serializer s;
    // 256-byte buffer with iota pattern (0,1,2,...,255)
    std::vector<uint8_t> iota_buf(256);
    std::iota(iota_buf.begin(), iota_buf.end(), static_cast<uint8_t>(0));
    s.write_bytes(iota_buf.data(), iota_buf.size());
    // Reversed copy
    std::vector<uint8_t> rev_buf(iota_buf.rbegin(), iota_buf.rend());
    s.write_bytes(rev_buf.data(), rev_buf.size());
    Deserializer d(s.data());
    auto r1 = d.read_bytes();
    ASSERT_EQ(r1.size(), 256u);
    for (size_t i = 0; i < 256; ++i) {
        EXPECT_EQ(r1[i], static_cast<uint8_t>(i));
    }
    auto r2 = d.read_bytes();
    ASSERT_EQ(r2.size(), 256u);
    for (size_t i = 0; i < 256; ++i) {
        EXPECT_EQ(r2[i], static_cast<uint8_t>(255 - i));
    }
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, U64LargeValuesAndBitPatternsV110) {
    Serializer s;
    s.write_u64(0xFFFFFFFFFFFFFFFFULL);
    s.write_u64(0x0000000100000000ULL);  // 2^32
    s.write_u64(0xAAAAAAAAAAAAAAAAULL);  // alternating bits
    s.write_u64(0x5555555555555555ULL);  // complement alternating
    s.write_u64(0xDEADBEEFCAFEBABEULL);
    s.write_u64(0x0102040810204080ULL);
    Deserializer d(s.data());
    EXPECT_EQ(d.read_u64(), 0xFFFFFFFFFFFFFFFFULL);
    EXPECT_EQ(d.read_u64(), 0x0000000100000000ULL);
    EXPECT_EQ(d.read_u64(), 0xAAAAAAAAAAAAAAAAULL);
    EXPECT_EQ(d.read_u64(), 0x5555555555555555ULL);
    EXPECT_EQ(d.read_u64(), 0xDEADBEEFCAFEBABEULL);
    EXPECT_EQ(d.read_u64(), 0x0102040810204080ULL);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, InterleavedTypesStressV110) {
    Serializer s;
    // Write a repeating pattern of all types, 5 iterations
    for (int i = 0; i < 5; ++i) {
        s.write_u8(static_cast<uint8_t>(i));
        s.write_u16(static_cast<uint16_t>(i * 1000));
        s.write_u32(static_cast<uint32_t>(i * 100000));
        s.write_u64(static_cast<uint64_t>(i) * 10000000000ULL);
        s.write_i32(-i);
        s.write_i64(static_cast<int64_t>(-i) * 10000000000LL);
        s.write_f64(static_cast<double>(i) * 1.5);
        s.write_bool(i % 2 == 0);
        s.write_string("iter_" + std::to_string(i));
        std::vector<uint8_t> buf(i + 1, static_cast<uint8_t>(i * 10));
        s.write_bytes(buf.data(), buf.size());
    }
    Deserializer d(s.data());
    for (int i = 0; i < 5; ++i) {
        EXPECT_EQ(d.read_u8(), static_cast<uint8_t>(i));
        EXPECT_EQ(d.read_u16(), static_cast<uint16_t>(i * 1000));
        EXPECT_EQ(d.read_u32(), static_cast<uint32_t>(i * 100000));
        EXPECT_EQ(d.read_u64(), static_cast<uint64_t>(i) * 10000000000ULL);
        EXPECT_EQ(d.read_i32(), -i);
        EXPECT_EQ(d.read_i64(), static_cast<int64_t>(-i) * 10000000000LL);
        EXPECT_DOUBLE_EQ(d.read_f64(), static_cast<double>(i) * 1.5);
        EXPECT_EQ(d.read_bool(), (i % 2 == 0));
        EXPECT_EQ(d.read_string(), "iter_" + std::to_string(i));
        auto blob = d.read_bytes();
        ASSERT_EQ(blob.size(), static_cast<size_t>(i + 1));
        for (size_t j = 0; j < blob.size(); ++j) {
            EXPECT_EQ(blob[j], static_cast<uint8_t>(i * 10));
        }
    }
    EXPECT_FALSE(d.has_remaining());
}
...
TEST(SerializerTest, U8BoundaryValuesAndBitwiseIdentityV111) {
    Serializer s;
    // Write all powers of 2 that fit in u8, plus 0 and 255
    s.write_u8(0);
    s.write_u8(1);
    s.write_u8(2);
    s.write_u8(4);
    ...
    s.write_u8(128);
    s.write_u8(255);
    // Also write the bitwise complement of a few values
    s.write_u8(static_cast<uint8_t>(~uint8_t(0)));   // 255
    s.write_u8(static_cast<uint8_t>(~uint8_t(1)));   // 254
    s.write_u8(static_cast<uint8_t>(~uint8_t(128))); // 127
    ...
    EXPECT_EQ(d.read_u8(), 2);
    EXPECT_EQ(d.read_u8(), 4);
    EXPECT_EQ(d.read_u8(), 8);
    EXPECT_EQ(d.read_u8(), 16);
    EXPECT_EQ(d.read_u8(), 32);
    EXPECT_EQ(d.read_u8(), 64);
    EXPECT_EQ(d.read_u8(), 128);
    EXPECT_EQ(d.read_u8(), 255);
    EXPECT_EQ(d.read_u8(), 255);
    EXPECT_EQ(d.read_u8(), 254);
...
TEST(SerializerTest, U16LittleEndianPatternVerificationV111) {
    Serializer s;
    // Write u16 values where high and low bytes are distinct
    s.write_u16(0x0000);
    s.write_u16(0x00FF);
    s.write_u16(0xFF00);
    s.write_u16(0xFFFF);
    s.write_u16(0xABCD);
    s.write_u16(0x1234);
    s.write_u16(0x8000);  // sign bit for i16
    s.write_u16(0x7FFF);  // max positive i16

    const auto& data = s.data();
    // Verify raw data is present and total size is 8 * 2 = 16 bytes
    ...
    EXPECT_EQ(d.read_u16(), 0x1234);
    EXPECT_EQ(d.read_u16(), 0x8000);
    EXPECT_EQ(d.read_u16(), 0x7FFF);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, I32I64SignedArithmeticEdgeCasesV111) {
    Serializer s;
    // i32 edge cases
    s.write_i32(std::numeric_limits<int32_t>::min());
    s.write_i32(std::numeric_limits<int32_t>::max());
    ...
    s.write_i32(1);
    s.write_i32(std::numeric_limits<int32_t>::min() + 1);
    s.write_i32(std::numeric_limits<int32_t>::max() - 1);
    // i64 edge cases
    s.write_i64(std::numeric_limits<int64_t>::min());
    s.write_i64(std::numeric_limits<int64_t>::max());
    s.write_i64(-1);
    s.write_i64(0);
    s.write_i64(1);
    s.write_i64(std::numeric_limits<int64_t>::min() + 1);
    s.write_i64(std::numeric_limits<int64_t>::max() - 1);
    ...
    EXPECT_EQ(d.read_i32(), 1);
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::min() + 1);
    EXPECT_EQ(d.read_i32(), std::numeric_limits<int32_t>::max() - 1);
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::min());
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::max());
    EXPECT_EQ(d.read_i64(), -1);
    EXPECT_EQ(d.read_i64(), 0);
    EXPECT_EQ(d.read_i64(), 1);
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::min() + 1);
    EXPECT_EQ(d.read_i64(), std::numeric_limits<int64_t>::max() - 1);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, F64SpecialIEEEValuesAndPrecisionV111) {
    Serializer s;
    s.write_f64(0.0);
    s.write_f64(-0.0);
    s.write_f64(std::numeric_limits<double>::infinity());
    ...
    s.write_f64(std::numeric_limits<double>::quiet_NaN());
    s.write_f64(std::numeric_limits<double>::min());        // smallest positive normal
    s.write_f64(std::numeric_limits<double>::denorm_min()); // smallest positive subnormal
    s.write_f64(std::numeric_limits<double>::max());
    s.write_f64(-std::numeric_limits<double>::max());
    s.write_f64(std::numeric_limits<double>::epsilon());
    // A value that exercises full mantissa precision
    s.write_f64(1.0 / 3.0);
    s.write_f64(3.141592653589793238);

    Deserializer d(s.data());
    EXPECT_DOUBLE_EQ(d.read_f64(), 0.0);
    // -0.0 compares equal to 0.0 with ==, verify bit pattern via signbit
    double neg_zero = d.read_f64();
    EXPECT_TRUE(std::signbit(neg_zero));
    EXPECT_DOUBLE_EQ(neg_zero, 0.0);
    EXPECT_TRUE(std::isinf(d.read_f64()));    // +inf
    double neg_inf = d.read_f64();
    EXPECT_TRUE(std::isinf(neg_inf));
    EXPECT_TRUE(neg_inf < 0.0);
    EXPECT_TRUE(std::isnan(d.read_f64()));     // NaN
    EXPECT_DOUBLE_EQ(d.read_f64(), std::numeric_limits<double>::min());
    EXPECT_DOUBLE_EQ(d.read_f64(), std::numeric_limits<double>::denorm_min());
    EXPECT_DOUBLE_EQ(d.read_f64(), std::numeric_limits<double>::max());
    EXPECT_DOUBLE_EQ(d.read_f64(), -std::numeric_limits<double>::max());
    EXPECT_DOUBLE_EQ(d.read_f64(), std::numeric_limits<double>::epsilon());
    EXPECT_DOUBLE_EQ(d.read_f64(), 1.0 / 3.0);
    EXPECT_DOUBLE_EQ(d.read_f64(), 3.141592653589793238);
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BoolAlternatingPatternAndBatchV111) {
    Serializer s;
    // Write an alternating pattern of bools
    for (int i = 0; i < 20; ++i) {
        s.write_bool(i % 2 == 0);
    }
    // Write a block of all-true and all-false
    for (int i = 0; i < 10; ++i) {
        s.write_bool(true);
    }
    for (int i = 0; i < 10; ++i) {
    ...
    Deserializer d(s.data());
    for (int i = 0; i < 20; ++i) {
        EXPECT_EQ(d.read_bool(), (i % 2 == 0)) << "alternating index " << i;
    }
    for (int i = 0; i < 10; ++i) {
        EXPECT_TRUE(d.read_bool()) << "all-true index " << i;
    }
    for (int i = 0; i < 10; ++i) {
        EXPECT_FALSE(d.read_bool()) << "all-false index " << i;
    }
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, StringVariousLengthsAndUnicodeV111) {
    Serializer s;
    s.write_string("");                            // empty string
    s.write_string("a");                           // single char
    s.write_string(std::string(1000, 'X'));         // long string
    s.write_string("Hello, World!");               // ASCII
    s.write_string("Caf\xC3\xA9");                // UTF-8: "Cafe"
    s.write_string("\xE4\xB8\xAD\xE6\x96\x87");  // UTF-8: Chinese
    s.write_string("\xF0\x9F\x98\x80");            // UTF-8: emoji
    s.write_string(std::string(1, '\0'));           // string containing a null byte
    ...
    EXPECT_EQ(d.read_string(), "Hello, World!");
    EXPECT_EQ(d.read_string(), "Caf\xC3\xA9");
    EXPECT_EQ(d.read_string(), "\xE4\xB8\xAD\xE6\x96\x87");
    EXPECT_EQ(d.read_string(), "\xF0\x9F\x98\x80");
    std::string null_str = d.read_string();
    EXPECT_EQ(null_str.size(), 1u);
    EXPECT_EQ(null_str[0], '\0');
    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, BytesBlobsWithPatternedDataV111) {
    Serializer s;
    // Empty blob
    s.write_bytes(nullptr, 0);
    // Single byte blob
    ...
    s.write_bytes(&one_byte, 1);
    // Ascending pattern blob (256 bytes)
    std::vector<uint8_t> ascending(256);
    std::iota(ascending.begin(), ascending.end(), static_cast<uint8_t>(0));
    s.write_bytes(ascending.data(), ascending.size());
    // Large blob with repeating pattern
    std::vector<uint8_t> large(4096);
    for (size_t i = 0; i < large.size(); ++i) {
        large[i] = static_cast<uint8_t>((i * 7 + 3) % 256);
    }
    s.write_bytes(large.data(), large.size());
    // All-zero blob
    std::vector<uint8_t> zeros(128, 0x00);
    s.write_bytes(zeros.data(), zeros.size());
    // All-0xFF blob
    std::vector<uint8_t> ones(128, 0xFF);
    s.write_bytes(ones.data(), ones.size());

    Deserializer d(s.data());
    auto empty_blob = d.read_bytes();
    EXPECT_TRUE(empty_blob.empty());

    auto single_blob = d.read_bytes();
    ASSERT_EQ(single_blob.size(), 1u);
    EXPECT_EQ(single_blob[0], 0xAB);

    auto asc_blob = d.read_bytes();
    ASSERT_EQ(asc_blob.size(), 256u);
    for (size_t i = 0; i < 256; ++i) {
        EXPECT_EQ(asc_blob[i], static_cast<uint8_t>(i)) << "ascending index " << i;
    }

    auto large_blob = d.read_bytes();
    ASSERT_EQ(large_blob.size(), 4096u);
    for (size_t i = 0; i < large_blob.size(); ++i) {
        EXPECT_EQ(large_blob[i], static_cast<uint8_t>((i * 7 + 3) % 256)) << "large index " << i;
    }

    auto zero_blob = d.read_bytes();
    ASSERT_EQ(zero_blob.size(), 128u);
    for (auto b : zero_blob) EXPECT_EQ(b, 0x00);

    auto ff_blob = d.read_bytes();
    ASSERT_EQ(ff_blob.size(), 128u);
    for (auto b : ff_blob) EXPECT_EQ(b, 0xFF);

    EXPECT_FALSE(d.has_remaining());
}

TEST(SerializerTest, MixedTypeInterleavedProtocolSimulationV111) {
    // Simulate a protocol-like message: header (u8 version, u16 msg_type, u32 seq),
    // then payload (string key, i64 timestamp, f64 value, bool valid, bytes raw_data)
    // repeated for multiple "messages"
    Serializer s;
    for (int msg = 0; msg < 5; ++msg) {
        // Header
        s.write_u8(static_cast<uint8_t>(1 + msg));                // version
        s.write_u16(static_cast<uint16_t>(100 + msg * 10));       // msg_type
        s.write_u32(static_cast<uint32_t>(1000 + msg));           // sequence
        s.write_u64(static_cast<uint64_t>(msg) * 999999999999ULL); // request_id
        // Payload
        s.write_string("key_" + std::to_string(msg));
        s.write_i32(-(msg + 1) * 1000);
        s.write_i64(1700000000000LL + static_cast<int64_t>(msg) * 86400000LL);
        s.write_f64(static_cast<double>(msg) * 0.123456789);
        s.write_bool(msg % 3 != 0);
        // Raw payload bytes
        std::vector<uint8_t> raw(msg * 10 + 5);
        for (size_t i = 0; i < raw.size(); ++i) {
            raw[i] = static_cast<uint8_t>((msg * 17 + i * 3) % 256);
        }
        s.write_bytes(raw.data(), raw.size());
    }

    Deserializer d(s.data());
    for (int msg = 0; msg < 5; ++msg) {
        EXPECT_EQ(d.read_u8(), static_cast<uint8_t>(1 + msg)) << "msg " << msg;
        EXPECT_EQ(d.read_u16(), static_cast<uint16_t>(100 + msg * 10)) << "msg " << msg;
        EXPECT_EQ(d.read_u32(), static_cast<uint32_t>(1000 + msg)) << "msg " << msg;
        EXPECT_EQ(d.read_u64(), static_cast<uint64_t>(msg) * 999999999999ULL) << "msg " << msg;
        EXPECT_EQ(d.read_string(), "key_" + std::to_string(msg)) << "msg " << msg;
        EXPECT_EQ(d.read_i32(), -(msg + 1) * 1000) << "msg " << msg;
        EXPECT_EQ(d.read_i64(), 1700000000000LL + static_cast<int64_t>(msg) * 86400000LL) << "msg " << msg;
        EXPECT_DOUBLE_EQ(d.read_f64(), static_cast<double>(msg) * 0.123456789) << "msg " << msg;
        EXPECT_EQ(d.read_bool(), (msg % 3 != 0)) << "msg " << msg;
        auto raw = d.read_bytes();
        ASSERT_EQ(raw.size(), static_cast<size_t>(msg * 10 + 5)) << "msg " << msg;
        for (size_t i = 0; i < raw.size(); ++i) {
            EXPECT_EQ(raw[i], static_cast<uint8_t>((msg * 17 + i * 3) % 256))
                << "msg " << msg << " byte " << i;
        }
    }
    EXPECT_FALSE(d.has_remaining());
